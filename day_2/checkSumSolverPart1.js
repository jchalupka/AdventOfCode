function checkSumSolver(table) {
  ...
  const result = rows
    .reduce((sum, cur) => {
      ...
      // add the largest and smallest
      const min = elems[0];
      const max = elems[elems.length - 1];

      console.log(`adding the min: ${min} and max: ${max}`);
      return sum + (max - min);
    }, 0);
