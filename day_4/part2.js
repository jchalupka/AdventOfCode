function passPhraseValidity(phrase) {
  ...
  const uniqueWords = [];
  const valid = words.map((word) => {
    const alphaWord = sortAlphabet(word);
    const isUnique = uniqueWords.map((uniqueWord) => {
      if (alphaWord === uniqueWord) {
        return false;
      }
    ...
    if (isUnique.includes(false)) {
      return false;
    }

    uniqueWords.push(alphaWord);
...
function getNumValid(phrases) {
  return phrases
    .split('\n')
    .reduce((total, cur) => {
      const valid = passPhraseValidity(cur);
      return (total + valid);
    }, 0);
}
...
const result = getNumValid(input);
console.log(`result ${result}`);

module.exports = passPhraseValidity;
